...
public class BinarySearchTree<T> extends AbstractSet<T> {
    ...
    public float averageDepth() {
        int totalDepth = recursiveAverageDepth(root, 0);
        return ((float) totalDepth) / size;
    }

    private int recursiveAverageDepth(Node<T> node, int depth) {
        if (node == null) {
            return 0;
        }
        return depth + recursiveAverageDepth(node.left, depth + 1) + recursiveAverageDepth(node.right, depth + 1);
    }
